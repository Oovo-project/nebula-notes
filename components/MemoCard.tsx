...
function chipStyles(category: RecentMemo["category"]) {
...
export default function MemoCard({ memo }: { memo: RecentMemo }) {
  return (
    <Link
      href={`/memo/${memo.id}`}
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <time className="text-[12px] text-[var(--text-dim)]">{formatDateLabel(memo.createdAt)}</time>
      </div>
